...
function kmeans2D(data: number[][], k: number, seed = 42) {
  ...
  for (let iter = 0; iter < 25; iter++) {
    ...
    const next = centroids.map((c, j) =>
      sums[j][2] ? [sums[j][0] / sums[j][2], sums[j][1] / sums[j][2]] : c,
    );
    if (
      centroids.every(
        (c, j) =>
          Math.abs(c[0] - next[j][0]) < 1e-9 &&
          Math.abs(c[1] - next[j][1]) < 1e-9,
      )
    )
      break;
    centroids = next;
...
export function supertrendAI(
  ...
  const alpha = Math.min(1, Math.max(0.0001, perfAlpha));
  const step = Math.max(1e-6, factorStep);
  const fmin = Math.min(factorMin, factorMax);
  const fmax = Math.max(factorMin, factorMax);
  const k = Math.max(1, Math.floor(opts.k ?? 3));
  const seed = opts.seed ?? 42;

  const factors: number[] = [];
  for (let f = fmin; f <= fmax + 1e-12; f += step) {
    factors.push(+f.toFixed(6));
  }
  ...
  const perFactor: PF[] = factors.map((f) => {
    ...
    for (let i = 1; i < bars.length; i++) {
      ...
      perf[i] = perf[i - 1] + alpha * (dC * signal[i - 1] - perf[i - 1]);
    }
    return { factor: f, bands, signal, perf };
  });

  const outBands: STPoint[] = [];
  const outFactor: number[] = [];
  const outPerf: number[] = [];
  const outCluster: ('LOW' | 'AVG' | 'TOP')[] = [];

  if (bars.length > 0) {
    outBands.push({ t: bars[0].t, upper: Number.NaN, lower: Number.NaN, trend: 1 });
    outFactor.push(Number.NaN);
    outPerf.push(Number.NaN);
    outCluster.push('TOP');
  }
  ...
  for (let i = 1; i < bars.length; i++) {
    ...
    const order = centroids
      .map((c, idx) => ({ idx, mu: c[0] }))
      .sort((a, b) => b.mu - a.mu)
      .map((x) => x.idx);

    const top = order[0];
    const second = order[1] ?? order[0];

    const topF: number[] = [];
    const secondF: number[] = [];
    const topP: number[] = [];
    const secondP: number[] = [];

    labels.forEach((cid, j) => {
      const f = perFactor[j].factor;
      const p = feats[j][0];
      if (cid === top) {
        topF.push(f);
        topP.push(p);
    ...
    const fTop = mean(topF);
    const fSec = mean(secondF);
    const pTop = Math.abs(mean(topP)) + 1e-9;
    const pSec = Math.abs(mean(secondP)) + 1e-9;

    const fStar = !Number.isFinite(fTop)
      ? fSec
      : !Number.isFinite(fSec)
          ? fTop
          : (pTop * fTop + pSec * fSec) / (pTop + pSec);

    const bands = supertrendBands(bars.slice(0, i + 1), atrPeriod, fStar);
    const last = bands[bands.length - 1];

    outBands.push(last);
    outFactor.push(fStar);
    outPerf.push(pTop);
    outCluster.push('TOP');
  }
