// Base Supertrend bands with ATR as RMA of True Range (SPEC-1A).
export function supertrendBands(bars, atrPeriod = 10, factor = 3) {
    const n = bars.length;
    if (!n)
        return [];
    ...
    for (let i = 0; i < n; i++) {
        ...
        const range = Math.max(bars[i].h - bars[i].l, Math.abs(bars[i].h - prevClose), Math.abs(bars[i].l - prevClose));
        tr[i] = range;
