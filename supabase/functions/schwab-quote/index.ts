...
async function fetchQuotes(symbols: string[], token: OAuthToken): Promise<Quote[]> {
  ...
  return symbols
    .map((symbol) => {
      ...
      return {
        symbol,
        bidPrice: Number(payload.bidPrice ?? payload.bid ?? 0),
        askPrice: Number(payload.askPrice ?? payload.ask ?? 0),
        lastPrice: Number(payload.lastPrice ?? payload.last ?? 0),
        volume: Number(payload.totalVolume ?? payload.volume ?? 0),
        timestamp: Number(payload.quoteTime ?? payload.timestamp ?? Date.now()),
      };
    })
